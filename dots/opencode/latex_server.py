...
Vendored from https://github.com/RobertoDure/mcp-latex-server
Run with: uv run latex_server.py [--base-path /path/to/latex/files]
"""
...
class LaTeXServer:
            ...
            try:
                if uri.startswith("file://"):
                    ...
                    if file_path.suffix == ".tex" and file_path.exists():
                        ...
                        raise ValueError(
                            f"File not found or not a LaTeX file: {file_path}"
                        )
                else:
    ...
    def _get_file_path(self, file_path: str) -> Path:
        ...
        try:
            ...
            raise ValueError(
                f"Path {file_path} is outside the allowed base path ({allowed_base})"
            )
    ...
    async def _create_latex_file(
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Error creating LaTeX file: {str(e)}")
            ]

    async def _edit_latex_file(
        self,
        file_path: str,
        operation: str,
        new_text: str,
        ...
        try:
            ...
            if operation == "replace":
                if not search_text:
                    return [
                        TextContent(
                            type="text",
                            text="search_text is required for replace operation",
                        )
                    ]
                ...
                if search_text:
                    ...
                    return [
                        TextContent(
                            type="text",
                            text="search_text or valid line_number is required",
                        )
                    ]
                ...
                return [
                    TextContent(type="text", text=f"Unknown operation: {operation}")
                ]
            ...
            return [
                TextContent(type="text", text=f"Error editing LaTeX file: {str(e)}")
            ]
    ...
    async def _read_latex_file(self, file_path: str) -> List[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Error reading LaTeX file: {str(e)}")
            ]

    async def _list_latex_files(
        ...
        try:
            ...
            if not tex_files:
                return [
                    TextContent(type="text", text=f"No LaTeX files found in {dir_path}")
                ]
            ...
            return [
                TextContent(type="text", text=f"Error listing LaTeX files: {str(e)}")
            ]
    ...
    async def _validate_latex(self, file_path: str) -> List[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(type="text", text=f"Error validating LaTeX file: {str(e)}")
            ]
    ...
    async def _get_latex_structure(self, file_path: str) -> List[TextContent]:
        ...
        try:
            ...
            return [
                TextContent(
                    type="text", text=f"Structure of {path}:\n\n" + "\n".join(structure)
                )
            ]
            ...
            return [
                TextContent(
                    type="text", text=f"Error analyzing LaTeX structure: {str(e)}"
                )
            ]
